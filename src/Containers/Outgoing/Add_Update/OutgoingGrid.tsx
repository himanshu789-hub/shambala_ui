...
function CalculateNetPrice(data: OutgoingUpdateRow, pricePerBottle: number) {
    ...
    const saleMinusCustomPriceValue = addDecimal(saleMinusCustomPrice, data.CustomCaratPrices.TotalPrice);
    const value = addDecimal(saleMinusCustomPriceValue, -data.SchemeInfo.TotalSchemePrice);
    return value;
}
...
const updateColDefs: (ColDef | ColGroupDef)[] = [
    ...
    },
    {
        ...
        valueSetter: function (params: ValueSetterParams<number>) {
            ...
            params.data.Shipment.TotalShipedPrice = getTotalPrice(params.newValue, product.PricePerCaret, product.PricePerBottle, product.CaretSize);
            return true;
        ...
        cellRendererFramework: QuantityWithPriceCellRenderer((params: CellRendererParams<number>) => params.data.Shipment.TotalQuantityShiped, (parsms: CellRendererParams<number>) => parsms.data.Shipment.TotalShipedPrice, (params: CellRendererParams<number>) => params.data.Shipment.CaretSize)
    },
    {
        headerName: 'Scheme',
        children: [
            {
                ...
                onCellValueChanged(params: CellValueChangedEvent<SchemeInfo>) {
                    //@ts-ignore
                    const schemeProduct = params.context.getProductDetails(window.config.SchemeProductId);
        ...
        valueSetter: (params: ValueSetterParams<OutgoingUpdateRow['CustomCaratPrices']['Prices']>) => {
            ...
            for (const price of params.newValue) {
                ...
                params.data.Shipment.CustomCaratPrices.TotalPrice = addDecimal(params.data.Shipment.CustomCaratPrices.TotalPrice,
                    getTotalPrice(price.Quantity, price.PricePerCarat, pricePerBottle, product.CaretSize));
            }
...
export default class OutgoingGrid extends React.Component<OutgoingGridProps, OutgoingGridState>{
    ...
    constructor(props: OutgoingGridProps) {
        ...
        this.state = {
            ...
            IsOnUpdate,
            OutgoingData: { DateCreated: new Date().toISOString(), SalesmanId: -1, Shipments: [], Id: 0, RowVersion: 0 },
        };
    }
    ...
    createAShipment = (componentId: number): OutgoingGridRowValue => {
        return {
            ...
            Shipment: {
                ...
                NetPrice: 0, UnitPrice: 0, TotalShipedPrice: 0
            }
        }
    }
    addAShipment = () => {
    ...
    IsFormValid = () => {
        ...
        if (!this.state.GridOptions.api) {
            alert('Grid Api Not Set.\nPlease, Contact Administration');
            isValid = false;
        }
        if (this.state.GridOptions.api?.getDisplayedRowCount() == 0) {
    ...
    handleSubmit = () => {
        ...
        if (!IsOnUpdate) {
            ...
            this.outgoingService
                .UpdateOutgoingShipment({ DateCreated: new Date().toISOString(), SalesmanId: OutgoingData.SalesmanId, Id: OutgoingData.Id, OutgoingShipmentDetails: details, RowVersion: OutgoingData.RowVersion })
                        ...
                        return this.productService.GetAll(res.data.DateCreated).then(res2 => {
                            ...
                            res.data.OutgoingShipmentDetails.forEach(element => {
                                ...
                                rowData.push({
                                    ...
                                    Observer: observer,
                                    Shipment: {
                                        ...element, Status: OutgoingGridRowCode.NONE, UnitPrice: product?.PricePerCaret ?? 0, CaretSize: product?.CaretSize ?? 0,
                                        TotalShipedPrice: res.data.Status === OutgoingStatus.PENDING ? price : element.TotalShipedPrice,
                                        NetPrice: res.data.Status === OutgoingStatus.PENDING ? price : element.NetPrice
                                    }
                                })
                            });
